...
class EncryptTheMessage
{
    ...
    private static void EncryptMessage(string recieveMessage)
    {
        ...
        {
            if (recieveMessage[i] >= 'A' && recieveMessage[i] <= 'M' || recieveMessage[i] >= 'a' && recieveMessage[i] <= 'm')
            {
                currentMessage += (Convert.ToString((char)(recieveMessage[i] + 13)));
            }
            else if (recieveMessage[i] >= 'N' && recieveMessage[i] <= 'Z' || recieveMessage[i] >= 'n' && recieveMessage[i] <= 'z')
            {
                currentMessage += (Convert.ToString((char)(recieveMessage[i] - 13)));
            }
