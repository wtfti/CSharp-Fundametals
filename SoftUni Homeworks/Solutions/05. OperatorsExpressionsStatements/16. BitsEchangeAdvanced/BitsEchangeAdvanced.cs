...
class BitsEchangeAdvanced
{
    public static bool[] pToNPositionsBools;
    public static bool[] QToNPositionsBools;
    ...
    static void Main()
    {
        ...
        if (p < 0 || p > numberToBit.Length || q < 0 || q > numberToBit.Length || nPositions <= 0)
        {
        ...
        {
            p = (numberToBit.Length - p)-nPositions;
            q = (numberToBit.Length - q)-nPositions;
            bits = new BitArray(numberToBit.Length);
            pToNPositionsBools = new bool[nPositions];
            QToNPositionsBools = new bool[nPositions];
            FillBitArray(bits, numberToBit);
            GetSetBitBoolValue(p, p + nPositions, QToNPositionsBools, "Get");
            GetSetBitBoolValue(q, q + nPositions, pToNPositionsBools, "Get");
            GetSetBitBoolValue(p, p + nPositions, pToNPositionsBools, "Set");
            GetSetBitBoolValue(q, q + nPositions, QToNPositionsBools, "Set");
